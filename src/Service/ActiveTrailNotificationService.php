...
class ActiveTrailNotificationService
{
    ...
    public function __construct(ParameterBagInterface $parameterBag, LoggerInterface $logger)
    {
        ...
        // dump($this->apiKey, $this->listId); // Assure-toi que les valeurs sont bonnes

        // Si tu veux tester le client
        // dump($this->activeTrailClient); exit();
    }
    ...
    public function sendNotification(string $missionTitle, string $missionDescription)
    {
        ...
        try {
            $response = $this->client->request('POST', 'https://api.activetrail.com/v2/campaigns/send', [
                'json' => [
                    'api_key' => $this->apiKey,
                    'list_id' => $this->listId,
                    'subject' => $subject,
